...
namespace Reporting.RdlAsp
{
 ...
 //               if (!_NoShow) { report = GetCachedReport(file); }
                ...
                ReportHelper.SetUserContext(report, this.Context, new NeedPassword(GetPassword));
		...
		private Report GetReport(string prog, string file)
		{
			...
			try
			{
				...
				if (r != null)
				{
                    ...
                    r.GetDataSourceReferencePassword = new NeedPassword(GetPassword);
				}
			}
			catch(Exception e)
			{
                ...
                else
                {
                    ...
                    for (int i = 0; i < rp.DisplayValues.Length; i++)
                    {
                        ...
                        if (rp.DataValues[i] is String &&
                            ...
                            pHtml.AppendFormat("<option{1}>{0}</option>", Xml.ToXmlAnsi(rp.DisplayValues[i]), selected);
                        else
                            pHtml.AppendFormat("<option value=\"{0}\"{2}>{1}</option>", Xml.ToXmlAnsi(rp.DataValues[i].ToString()), Xml.ToXmlAnsi(rp.DisplayValues[i]), selected);
                    }
