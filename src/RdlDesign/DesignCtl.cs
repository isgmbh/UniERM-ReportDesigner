...
namespace Reporting.RdlDesign
{
	...
	{ 
        ...
        private Point _ptLBLastClick = new Point();
		...
		{
            Point p = new Point();
            p.X = (int)PointsX(_ptLBLastClick.X + _hScroll.Value);
            p.Y = (int)PointsY(_ptLBLastClick.Y + _vScroll.Value);
        ...
        {
            Rectangle ph, pb, pf;
            ph = _DrawPanel.PageHeaderBounds;
            pb = _DrawPanel.BodyBounds;
            pf = _DrawPanel.PageFooterBounds;

            if (ph.Contains(point))
            {
                PointF p = new PointF(point.X - ph.Left, point.Y - ph.Top);
                DoPaste(_DrawPanel.PageHeader, p);
            }
            else if (pb.Contains(point))
            {
                PointF p = new PointF(point.X - pb.Left, point.Y - pb.Top);
                DoPaste(_DrawPanel.Body, p);
            }
            else if (pf.Contains(point))
            {
                PointF p = new PointF(point.X - pf.Left, point.Y - pf.Top);
                DoPaste(_DrawPanel.PageFooter, p);
            }
            else
            {
		...
		private void DrawPanelMouseUp(object sender, MouseEventArgs e)
		{
			...
			{	// Handle the end of the rubber banding
                ...
                //Save last click point if it is in the body
                //if (_DrawPanel.IsHitInBody(new PointF(e.X, e.Y)))
                //{
                    _ptLBLastClick.X = e.X;
                    _ptLBLastClick.Y = e.Y;
                //}
