/*
    ...
    and followed by a desciprtion of the change.
		
	Several gets and sets of properties have had 
	[System.Diagnostics.DebuggerStepThrough]
	applied to them to aid in debugging.  

	Added fields neccessary to add a border around the band, 
	as well as the fields for the color "behind" the page.
	
	Increased left page gap to 10f from 0f and made it public so it can be user
        with the RdlEditPreview top ruler offset.

	Adds support for "out of bounds" items to still display an outline
	Confines the "white paper" portion of the page displayed to 
		a maximum of the selected paper size (displayed work area will not exceed
		the paper size).
	Adds support for overlapping items and by extension, back ground images.
	
	Corrects issues with the image scaling when using large images.
	Implemented the TODO for Advanced Line Collision
	Adds ability to select "below" the current item by holding SHIFT when
	clicking. This allows selecting the base form when attempting to add new
	items over a "background" image.
	
	Added ReportItemReverseSorter class.
*/
/*
...
namespace Reporting.RdlDesign
{
	...
	internal class DesignXmlDraw: UserControl
	{
        ...
        // Josh: Made public so it can be used for the top ruler offset.
		public const float LEFTGAP = 10f;				// keep a gap on the left size of the screen
        //Josh: Changed leftgap from 0 to 10
        ...
        internal Rectangle PageHeaderBounds
        {
            [System.Diagnostics.DebuggerStepThrough]
            get
            {
                ...
                int y = 0;
                int w = this.Width;
                int h = (int)PageHeaderHeight;
                return new Rectangle(x, y, w, h);
            }
        }

        internal Rectangle BodyBounds
        {
            [System.Diagnostics.DebuggerStepThrough]
            get
            {
                int x = (int)LEFTGAP;
                int y = PageHeaderBounds.Bottom + BANDHEIGHT;
                int w = this.Width;
                int h = (int)BodyHeight;
                return new Rectangle(x, y, w, h);
            }
        }

        internal Rectangle PageFooterBounds
        {
            [System.Diagnostics.DebuggerStepThrough]
            get
            {
                int x = (int)LEFTGAP;
                int y = BodyBounds.Bottom + BANDHEIGHT;
                int w = this.Width;
                int h = (int)PageFooterHeight;
                return new Rectangle(x, y, w, h);
            }
        }
		...
		internal void Draw(Graphics ag, float hScroll, float vScroll, System.Drawing.Rectangle clipRectangle)
		{
            ...
            //Josh: Idiot me decided it would be best to 
            // include the bottom region so that the page
            // would have the background behind it.
            // I just realized that the _clip had the same rectangle
            // that needed to be filled in. DrawMargins has been
            ...
            //Used for testing rectange bounds
            //Pen pen = new Pen(new SolidBrush(Color.Red));
            //ag.DrawRectangle(pen, PageHeaderBounds);
            //ag.DrawRectangle(pen, BodyBounds);
            //ag.DrawRectangle(pen, PageFooterBounds);
		...
		private float DrawReportPrimaryRegions(XmlNode xNode, float xLoc, float yLoc, string title)
		{
            ...
            //float displayHeight;
            //displayHeight = ((height > TotalPageHeight /* - yLoc*/)? TotalPageHeight/* - yLoc*/ : height);

            //Josh: Fixed page not moving bug 
            // (When scrolling horizontally). 
            // For some reason, I was drawing the 
            // page the same width as it scrolled 
            // to the right. It's corrected by 
            // removing the _hScroll from the 
        ...
        private RectangleF DrawImage(XmlNode xNode, RectangleF r)
        {
            ...
            if (!ir.IntersectsWith(_clip))
                return ir;
            //ir.Intersect(_clip); //Josh: Removed so that the FitProportional 
                                   // would display correctly when full image 
        ...
        // Josh: Passing in the lower margin to fill in 
        // was unneccessary. That has been corrected.
        ...
        //Josh: Unneccessary work. The "background" rectangle was already obtained.
		...
		// return the BorderStyleEnum given a particular string value //Josh: Changed to use Enum.parse instead of switch
        [System.Diagnostics.DebuggerStepThrough]
...
//  TODO: this is problematic since it require a PageImage
        ...
        internal void SelectInRectangle(Rectangle r, float hScroll, float vScroll)
		{
            ...
            bh = BodyHeight;
			rf = new RectangleF(LEFTGAP, hh+BANDHEIGHT, PointsX(Width)+_hScroll, bh);
			if (rf.IntersectsWith(_HitRect))
			{
            ...
            fh = PageFooterHeight;
			rf = new RectangleF(LEFTGAP, hh+BANDHEIGHT+bh+BANDHEIGHT, PointsX(Width)+_hScroll, fh);
			if (rf.IntersectsWith(_HitRect))
			{
        ...
        public bool IsHitInBody(PointF p)
        {
            RectangleF rf = new RectangleF(LEFTGAP, PageHeaderHeight + BANDHEIGHT, PointsX(Width) + _hScroll, BodyHeight);
            if (rf.Contains(p))
            {
                return true;
            }
