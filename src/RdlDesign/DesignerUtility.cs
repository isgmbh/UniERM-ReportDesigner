...
namespace Reporting.RdlDesign
{
	...
	internal class DesignerUtility
	{
        ...
        static internal bool GetSharedConnectionInfo(IPasswordProvider pwdProvider, string filename, out string dataProvider, out string connectInfo)
        {
            dataProvider = null;
            connectInfo = null;
            ...
            catch
            {
                ...
                pwdProvider.ResetPassword();			// make sure to prompt again for the password
                return false;
            }
            XmlDocument xDoc = new XmlDocument();
            xDoc.LoadXml(xml);
...
//                        rowValues[ci++] = val;
        ...
        static internal bool GetConnnectionInfo(DesignXmlDraw d, string ds, out string dataProvider, out string connection)
        {
            ...
            if (dataSourceReference != null)
            {
                ...
                Control p = d;
                MDIChild mc = null;
                while (p != null && !(p is RdlDesignerForm))
                {
                    if (p is MDIChild)
                        mc = (MDIChild)p;
                ...
                if (!DesignerUtility.GetSharedConnectionInfo((RdlDesignerForm)p, filename.LocalPath , out dataProvider, out connection))
                    return false;
            }
            else
            {
	...
	internal class ReportParm
	{
        ...
        internal string DefaultDSRDataSetName
		{
            ...
            set
            {
                if (_DefaultDSRDataSetName != value)
		...
		internal string DefaultDSRValueField
		{
            ...
            set
            {
                if (_DefaultDSRValueField != value)
		...
		internal string ValidValuesDSRDataSetName
		{
            ...
            set
            {
                if (_ValidValuesDSRDataSetName != value)
		...
		internal string  ValidValuesDSRValueField
		{
            ...
            set
            {
                if (_ValidValuesDSRValueField != value)
		...
		internal string ValidValuesDSRLabelField
		{
            ...
            set
            {
                if (_ValidValuesDSRLabelField != value)
	...
	internal class StaticLists
	{
		...
		/// </summary>
		static public readonly string[] FunctionList = new string[] {
            "Iif(boolean, trueExpr, falseExpr)",
			"Choose(number, choice1 [,choice2]...)",
			"Switch(boolean1, value1[, boolean2, value2]...[elseValue])",
			"Format(expr, formatExpr)"};

		static public readonly string[] AggrFunctionList = new string[] {
            "Sum(number [, scope])",
