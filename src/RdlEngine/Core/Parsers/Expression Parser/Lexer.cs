...
/*
 * Josh: 6:22:10
 * This file had been modified to allow shortcuts for the 
 * parameters {?parameter} , fields {field}, user vars {!uservar}, and globals {@global}
*/
using System;
using System.IO;
	...
	internal class Lexer
	{
		...
		private Token ReadIdentifier(char ch, int initialState)
		{
            ...
            int state = initialState;      
                                // state=1 means accept letter,digit,'.','!','_'
                                // state=2 means accept whitespace ends with '.' or '!'
                                // state=3 means accept letter to start new qualifier
                                // state=4 means accept letter,digit,'@','?','!','_','}'
                                // Josh: 6:21:10 added state 4 for field/param shortcuts
            while (!reader.EndOfInput())
			{
				cPeek = reader.Peek();
                if (state == 1)
        ...
        // Josh 6:21:10 Added to allow using shortcuts for 
