	...
	internal class Parser
	{
		...
		internal IExpr Parse(NameLookup lu, string expr)
		{
			...
			if (expr.Substring(0,1) != "=")		// if 1st char not '='
				return new Constant<string>(expr);		//   this is a constant value //Originally Plain Constant

			idLookup = lu;	
			IExpr e = this.ParseExpr(new StringReader(expr));
			
			if (e == null)					// Didn't get an expression?
                e = new Constant<string>(expr);		//  then provide a constant //Originally Plain Constant
		...
		private void MatchBaseType(out IExpr result)
		{
			...
			switch (curToken.Type)
			{
				case TokenTypes.NUMBER:
					result = new Constant<decimal>(curToken.Value);
					break;
				case TokenTypes.DATETIME:
					result = new Constant<DateTime>(curToken.Value);
					break;
				case TokenTypes.DOUBLE:
					result = new Constant<double>(curToken.Value);
					break;
				case TokenTypes.INTEGER:
					result = new Constant<int>(curToken.Value);
					break;
				case TokenTypes.QUOTE:
					result = new Constant<string>(curToken.Value);
					break;
				default:
			...
			if (curToken.Type != TokenTypes.LPAREN) switch (firstPart)
			{
				...
				default:
					...
					switch (method.ToLower())		// lexer should probably mark these
					{
						case "true":
						case "false":
                            result = new Constant<Boolean>(method.ToLower());
...
//						throw new ParserException("First argument to iif function must be boolean." + "  At column " + Convert.ToString(curToken.StartCol));
				...
				case "format":
					...
					if (args.Length == 1)
					{
						result = new FunctionFormat(args[0], new Constant<string>(""));
					}
					else
					{
				...
				case "rownumber":
					scope = ResolveAggrScope(args, 1, out bSimple);
					IExpr texpr = new Constant<double>("0");
		...
		private IExpr ResolveMethodCall(string fullname, IExpr[] args)
		{
			...
			{
				argTypes[i] = Utility.Assembly.GetTypeFromTypeCode(args[i].GetTypeCode());
			}
			...
			if (cls == "" || cls.ToLower() == "code")
			{
				...
				if (cType != null)
				{
					if (Utility.Assembly.GetMethod(cType, method, argTypes) == null)
			...
			{	// ok try for some of the system functions
				...
				switch(cls)
				{
					...
					case "Convert":
						...
						break;
					case "Financial":
						syscls = "Reporting.Rdl.Financial";
						break;
					default:
						syscls = "Reporting.Rdl.VBFunctions";
						break;
				}
				if (syscls != null)
				{
            ...
            MethodInfo mInfo = Utility.Assembly.GetMethod(cType, method, argTypes);
            if (mInfo == null)
            {
                string err;
                if (cls == null || cls.Length == 0)
