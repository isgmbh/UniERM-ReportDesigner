	...
	internal class PageFooter : ReportLink
	{
			...
			foreach(XmlNode xNodeLoop in xNode.ChildNodes)
			{
				...
				switch (xNodeLoop.Name)
				{
					...
					case "PrintOnFirstPage":
						_PrintOnFirstPage = Conversion.ToBoolean(xNodeLoop.InnerText, OwnerReport.rl);
						break;
					case "PrintOnLastPage":
						_PrintOnLastPage = Conversion.ToBoolean(xNodeLoop.InnerText, OwnerReport.rl);
						break;
					case "ReportItems":
		...
		internal void RunPage(Pages pgs)
		{
			...
			foreach (Page p in pgs)
			{
				...
				p.YOffset = OwnerReport.PageHeight.ToPoints() 
									- OwnerReport.BottomMargin.ToPoints()
									- this._Height.ToPoints();
				p.XOffset = 0;
				pgs.CurrentPage = p;
				...
				_ReportItems.RunPage(pgs, null, OwnerReport.LeftMargin.ToPoints());
			}
		}
