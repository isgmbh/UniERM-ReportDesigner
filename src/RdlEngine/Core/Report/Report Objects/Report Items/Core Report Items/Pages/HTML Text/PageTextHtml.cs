	...
	public class PageTextHtml : PageText, IEnumerable, ICloneable
	{
        ...
        private PageImage BuildImage(Graphics g, string token, StyleInfo oldsi, PageText model)
        {
            ...
            try
            {
                ...
                pi.W = Measurement.PointsFromPixels(width != null? Convert.ToInt32(width): w, g.DpiX);
                pi.H = Measurement.PointsFromPixels(height != null? Convert.ToInt32(height): h, g.DpiY);
                pi.SI = new StyleInfo();
            }
            catch 
            {
        ...
        private void HandleFont(string token, StyleInfo model)
        {
            ...
            if (color != null && color.Length > 0)
                si.Color = Xml.ColorFromHtml(color, si.Color);
			...
			foreach (string item in styleList)
			{
				...
				switch (val[0].ToLower().Trim())
				{
					...
					case "background-color":
						si.BackgroundColor = Xml.ColorFromHtml(tval, si.Color);
						break;
					case "color":
						si.Color = Xml.ColorFromHtml(tval, si.Color);
						break;
					case "font-family":
						si.FontFamily = tval;
						break;
		...
		private SizeF MeasureString(string s, StyleInfo si, Graphics g, out float descent)
		{
			...
			try
			{
				...
				try
				{
					...
					descent = Measurement.PointsFromPixels(descentPixel, g.DpiX);
				}
				catch
				{
                ...
                ms.Height = Measurement.PointsFromPixels(mr.Height, g.DpiY);	// convert to points from pixels
                ms.Width = Measurement.PointsFromPixels(mr.Width, g.DpiX);		// convert to points from pixels
				return ms;
			}
			finally
			{
