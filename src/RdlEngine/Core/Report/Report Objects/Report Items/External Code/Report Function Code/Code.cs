...
namespace Reporting.Rdl
{
	...
	internal class Code : ReportLink
	{
		...
		private Assembly GetAssembly()
		{
            ...
            lines.Add("Imports Reporting.Rdl");
            lines.Add("namespace Reporting.vbgen");
...
//						tsw.Close();
            ...
            if (RdlEngineConfig.DirectoryLoadedFrom == null) {
                ...
                {
                    re = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, string.Format("Bin{0}RdlEngine.dll", Path.DirectorySeparatorChar));   // this can work especially in web scenarios
                }
            }
            else
		...
		internal Type CodeType()
		{
			...
			try
			{
				object instance = _Assembly.CreateInstance("Reporting.vbgen." + this._Classname, false); 
				t = instance.GetType();
			}
			catch (Exception e)
			{
		...
		internal object Load(Report rpt)
		{
			...
			try
			{
                ...
                args[0] = rpt;
				wc.Instance = _Assembly.CreateInstance("Reporting.vbgen." + this._Classname, false, BindingFlags.CreateInstance, null, args, null, null); 
			}
			catch (Exception e)
			{
				wc.Instance = null;
