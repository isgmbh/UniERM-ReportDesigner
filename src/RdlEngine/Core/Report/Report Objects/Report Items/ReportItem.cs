...
namespace Reporting.Rdl
{
	...
	internal class ReportItem : ReportLink, IComparable
	{
		...
		internal float HeightOrOwnerHeight
		{
			...
			{
				if (_Height != null)
					return _Height.ToPoints();

                float yloc = this.Top == null ? 0 : this.Top.ToPoints();
				...
				for (ReportLink rl = this.Parent; rl != null; rl = rl.Parent)
				{
					if (rl is ReportItem)
					{
						...
						if (ri.Height != null)
                            return ri.Height.ToPoints() - yloc;
						continue;
					}
					if (rl is PageHeader)
					{
						...
						if (ph.Height != null)
							return ph.Height.ToPoints() - yloc;
						continue;
					}
					if (rl is PageFooter)
					{
						...
						if (pf.Height != null)
                            return pf.Height.ToPoints() - yloc;
						continue;
					}
					if (rl is TableRow)
					{
						...
						if (tr.Height != null)
                            return tr.Height.ToPoints() - yloc;
						continue;
					}
					if (rl is MatrixRow)
					{
						...
						if (mr.Height != null)
                            return mr.Height.ToPoints() - yloc;
						continue;
					}
					if (rl is Body)
					{
						Body b = rl as Body;
						if (b.Height != null)
                            return b.Height.ToPoints() - yloc;
						continue;
					}
				}
                return OwnerReport.PageHeight.ToPoints();
			}
		}
		...
		internal int WidthCalc(Report rpt, Graphics g)
		{
			...
			{	// must be part of a matrix
				width = g==null? 
                    Measurement.PixelsFromPoints(wc.MC.Width, Measurement.STANDARD_DPI_X)
                    : 
                    Measurement.PixelsFromPoints(wc.MC.Width, g.DpiX);
			}
			else
			{	// not part of a table or matrix
				if (Width != null)
					...
					width = Measurement.PixelsFromPoints(WidthOrOwnerWidth(rpt), Measurement.STANDARD_DPI_X);
			}
			return width;
		}
		...
		internal Page RunPageNew(Pages pgs, Page p)
		{
			...
			{		// Stay on same page but move to new column
				p.XOffset = 
					((OwnerReport.Width.ToPoints() + b.ColumnSpacing.ToPoints()) * ccol);
				p.YOffset = top;
		...
		internal void SetPagePositionBegin(Pages pgs)
		{
			...
			{
				pgs.CurrentPage.YOffset = OwnerReport.TopMargin.ToPoints();

			}
			else if (this.Parent.Parent is PageFooter)
			{
				pgs.CurrentPage.YOffset = OwnerReport.PageHeight.ToPoints() 
					- OwnerReport.BottomMargin.ToPoints()
					- OwnerReport.PageFooter.Height.ToPoints();
			}
