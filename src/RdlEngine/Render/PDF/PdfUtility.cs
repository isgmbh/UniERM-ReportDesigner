//This file has been modified based on suggestions from forum users.
/*
...
{
	/// <summary>
	/// This class contains general Measurement for the creation of pdf
	/// Creates the Header
	/// Creates XrefTable
	/// Creates the Trailer
	/// </summary>
    internal class PdfUtility
    {
        ...
        internal byte[] CreateXrefTable(long fileOffset, out int size)
        {
            ...
            {
                Exception error = new Exception(e.Message + " In Measurement.CreateXrefTable()");
                throw error;
            }
        ...
        internal byte[] GetTrailer(int refRoot, int refInfo, out int size)
        {
            ...
            {
                Exception error = new Exception(e.Message + " In Measurement.GetTrailer()");
                throw error;
            }
        ...
        static internal byte[] GetUTF8Bytes(string str, out int size)
        {
            ...
            {
                Exception error = new Exception(e.Message + " In Measurement.GetUTF8Bytes()");
                throw error;
            }
        }
        ...
        */
        private static void appendQuotedOctet(StringBuilder buffer, byte octet)
        {
            ...
            else if (octet < ' ' || octet > '~')
            {
                ...
                // octal notation.
				// Fix provided by forum user _andreas_ http://www.fyireporting.com/forum/viewtopic.php?t=1010
                //FIX
                if (octet != 172)
                {
