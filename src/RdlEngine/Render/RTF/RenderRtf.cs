	...
	internal class RenderRtf: IPresent
	{
        ...
        private void PutPageMetrics(TextWriter ftw)
        {
            ftw.Write(@"\paperw{0}\paperh{1}\margl{2}\margr{3}\margt{4}\margb{5}",
                Measurement.TwipsFromPoints(r.PageWidthPoints),
                Measurement.TwipsFromPoints(r.PageHeightPoints),
...
\brdroutset Outset border.
        ...
        public bool MatrixStart(Matrix m, MatrixCellEntry[,] matrix, Row r, int headerRows, int maxRows, int maxCols)				// called first
		{
            ...
            for (int i = 0; i < maxCols; i++)
                _MatrixColumnWidths[i] = Measurement.TwipsFromPoints(widths[i]);
        ...
        public void Image(Image i, Row r, string mimeType, Stream ioin)
        {
            ...
            {
                PutImage(im, i.Width == null ? 0 : i.Width.ToPixels(), i.Height == null ? 0 : i.Height.ToPixels());
            }
        ...
        {
            return Measurement.TwipsFromPixels(pixels, GetGraphics.DpiX);
//            return (int)Math.Round(RSize.PointsFromPixels( pixels,GetGraphics.DpiX) * 20, 0);
        }
